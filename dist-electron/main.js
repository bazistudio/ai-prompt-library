...
var require_electron = __commonJS({
  "node_modules/electron/index.js"(exports2, module2) {
    ...
    var pathFile = path11.join(__dirname, "path.txt");
    function getElectronPath() {
      ...
      if (process.env.ELECTRON_OVERRIDE_DIST_PATH) {
        return path11.join(process.env.ELECTRON_OVERRIDE_DIST_PATH, executablePath || "electron");
      }
      if (executablePath) {
        return path11.join(__dirname, "dist", executablePath);
      } else {
...
var init_notifications = __esm({
  "electron/notifications.ts"() {
    "use strict";
    import_electron5 = __toESM(require_electron());
    init_logger();
  }
});
...
function cleanupOldBackups(backupPath) {
  try {
    const files = import_fs2.default.readdirSync(backupPath).filter((f) => f.startsWith("prompt_library_backup_") && f.endsWith(".db.gz")).map((f) => ({ name: f, time: import_fs2.default.statSync(import_path3.default.join(backupPath, f)).mtime.getTime() })).sort((a, b) => b.time - a.time);
    if (files.length > 30) {
...
async function restoreDatabase(backupFilePath) {
  try {
    ...
    const currentDbPath = import_path3.default.join(userDataPath, "prompt_library.db");
    const preRestorePath = import_path3.default.join(userDataPath, "prompt_library.db.pre-restore");
...
function createStartupSnapshot(dbPath) {
  if (import_fs2.default.existsSync(dbPath)) {
    try {
      ...
      const backupPath = import_path3.default.join(backupDir, `prompt_library_${timestamp}.db.backup`);
      import_fs2.default.copyFileSync(dbPath, backupPath);
      logger.info(`[BackupManager] Startup snapshot created at ${backupPath}`);
      const backups = import_fs2.default.readdirSync(backupDir).filter((f) => f.startsWith("prompt_library_") && f.endsWith(".db.backup")).sort();
      if (backups.length > 5) {
...
var init_backupManager = __esm({
  "electron/services/backupManager.ts"() {
    ...
    init_db();
    init_config();
    import_electron6 = __toESM(require_electron());
    init_logger();
    init_notifications();
  }
});
...
function initDb() {
  if (_db) return;
  const dbPath = import_path4.default.join(import_electron7.app.getPath("userData"), "prompt_library.db");
...
var init_db = __esm({
  "electron/db/index.ts"() {
    ...
    import_electron7 = __toESM(require_electron());
    init_schema();
    init_logger();
    init_backupManager();
    _db = null;
...
var init_platform = __esm({
  "node_modules/axios/lib/platform/index.js"() {
    ...
    init_node();
    init_utils2();
    platform_default = __spreadValues(__spreadValues({}, utils_exports), node_default);
  }
});
...
function parsePropPath(name) {
  ...
  while ((match = pattern.exec(name)) !== null) {
    ...
    path11.push(match[0] === "[]" ? "" : match[1] || match[0]);
  }
  return path11;
}
  ...
  "node_modules/axios/lib/helpers/Http2Sessions.js"() {
    ...
    Http2Sessions = class {
      ...
      getSession(authority, options) {
        ...
        const session2 = import_http2.default.connect(authority, options);
        let removed;
        let timer;
        const removeSession = () => {
          if (removed) {
...
function getTunnelingAgent(agentOptions, userHttpsAgent) {
  ...
  if (userHttpsAgent && userHttpsAgent.options) {
    const originTLSOptions = __spreadValues({}, userHttpsAgent.options);
    const callback = agent.callback;
    agent.callback = function axiosTunnelingAgentCallback(req, opts) {
      return callback.call(this, req, __spreadValues(__spreadValues({}, originTLSOptions), opts));
    };
  }
...
var init_http = __esm({
  "node_modules/axios/lib/adapters/http.js"() {
    ...
    isBrotliSupported = utils_default.isFunction(import_zlib2.default.createBrotliDecompress);
    isZstdSupported = utils_default.isFunction(import_zlib2.default.createZstdDecompress);
    ...
    http_default = isHttpAdapterSupported && function httpAdapter(config) {
      return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
        ...
        if (supportedProtocols.indexOf(protocol2) === -1) {
          return reject(
            new AxiosError_default("Unsupported protocol " + protocol2, AxiosError_default.ERR_BAD_REQUEST, config)
          );
        }
        ...
        if (socketPath) {
          ...
          if (allowedSocketPaths != null) {
            ...
            const resolvedSocket = (0, import_path5.resolve)(socketPath);
        ...
        req = transport.request(options, function handleResponse(res) {
          ...
          if (decompress !== false && res.headers["content-encoding"]) {
            ...
            switch ((res.headers["content-encoding"] || "").toLowerCase()) {
              ...
              case "compress":
              case "x-compress":
                streams.push(import_zlib2.default.createUnzip(zlibOptions));
                delete res.headers["content-encoding"];
                break;
              case "deflate":
...
var init_mergeConfig = __esm({
  "node_modules/axios/lib/core/mergeConfig.js"() {
    ...
    init_utils();
    init_AxiosHeaders();
    headersToObject = (thing) => thing instanceof AxiosHeaders_default ? __spreadValues({}, thing) : thing;
  ...
  "node_modules/axios/lib/adapters/xhr.js"() {
    ...
    xhr_default = isXHRAdapterSupported && function(config) {
      return new Promise(function dispatchXhrRequest(resolve, reject) {
        ...
        if (protocol2 && !platform_default.protocols.includes(protocol2)) {
          reject(
            new AxiosError_default(
...
var init_trackStream = __esm({
  "node_modules/axios/lib/helpers/trackStream.js"() {
    ...
    readBytes = function(iterable, chunkSize) {
      return __asyncGenerator(this, null, function* () {
        try {
    ...
    api = {
      ...
      syncEntity: async (entityType, operation, payload) => {
        ...
        try {
          ...
          return { success: true, data: response == null ? void 0 : response.data };
        } catch (error) {
...
function getLogPath() {
  if (!syncLogPath) {
    const logDir = import_path6.default.join(import_electron8.app.getPath("userData"), "logs");
    if (!import_fs3.default.existsSync(logDir)) {
      import_fs3.default.mkdirSync(logDir, { recursive: true });
    }
    syncLogPath = import_path6.default.join(logDir, "sync.log");
  }
  return syncLogPath;
}
var import_fs3, import_path6, import_electron8, syncLogPath, syncLogger;
var init_logger2 = __esm({
  "electron/services/logger.ts"() {
    "use strict";
    import_fs3 = __toESM(require("fs"));
    import_path6 = __toESM(require("path"));
    import_electron8 = __toESM(require_electron());
    syncLogPath = null;
    syncLogger = {
      info: (message, meta = {}) => {
...
`;
      ...
      error: (message, error) => {
        const entry = `[ERROR] ${(/* @__PURE__ */ new Date()).toISOString()} - ${message} - ${(error == null ? void 0 : error.message) || error}
`;
        import_fs3.default.appendFileSync(getLogPath(), entry);
      }
    };
...
var init_updater = __esm({
  "electron/updater.ts"() {
    ...
    import_electron14 = __toESM(require_electron());
    init_logger();
    init_syncEngine();
    init_db();
...
// electron/main.ts
var import_electron15 = __toESM(require_electron());
...
function createWindow() {
  ...
  const win = new import_electron3.BrowserWindow({
    ...
    icon: import_path2.default.join(__dirname, "../build/icon.ico"),
...
var import_electron4 = __toESM(require_electron());
init_logger();
...
function setupSecurity() {
  import_electron4.app.on("web-contents-created", (_event, contents) => {
    ...
    contents.setWindowOpenHandler(({ url: url2 }) => {
      logger.warn(`[security] Blocked window.open for: ${url2}`);
      import_electron4.shell.openExternal(url2);
      return { action: "deny" };
    });
  });
  import_electron4.app.whenReady().then(() => {
    ...
    ses.webRequest.onHeadersReceived((details, callback) => {
      ...
      callback({
        responseHeaders: __spreadProps(__spreadValues({}, details.responseHeaders), {
...
var import_electron10 = __toESM(require_electron());
...
var import_electron9 = __toESM(require_electron());
...
function setupIpcHandlers() {
  ...
  handleIpc("app:openLogs", () => {
    import_electron10.shell.openPath(import_path7.default.join(import_electron10.app.getPath("userData"), "logs"));
  });
...
var import_electron11 = __toESM(require_electron());
  ...
  import_electron11.app.on("render-process-gone", (event, webContents, details) => {
    ...
    if (details.reason === "crashed" || details.reason === "oom") {
      ...
      if (win && !win.isDestroyed()) {
        ...
        win.loadFile(import_path8.default.join(__dirname, "../electron/error.html")).catch((err) => {
...
function gracefulShutdown(reason, error) {
  ...
  if (!isDev3) {
    ...
    const logPath2 = import_path8.default.join(import_electron11.app.getPath("userData"), "logs", "main.log");
...
${error.stack}`);
  ...
  try {
    if ((_c = (_b = (_a = logger.transports) == null ? void 0 : _a.file) == null ? void 0 : _b.getFile()) == null ? void 0 : _c.clear) {
    }
  } catch (err) {
  }
...
// electron/splash.ts
var import_electron12 = __toESM(require_electron());
...
function createSplashWindow() {
  const win = new import_electron12.BrowserWindow({
    ...
    center: true,
    resizable: false,
    icon: import_path9.default.join(__dirname, "../build/icon.png"),
    webPreferences: {
      nodeIntegration: false,
      contextIsolation: true
    }
...
// electron/tray.ts
var import_electron13 = __toESM(require_electron());
...
function setupTray(mainWindow2) {
  ...
  const iconPath = import_electron13.app.isPackaged ? import_path10.default.join(process.resourcesPath, "app.asar", "build", iconName) : import_path10.default.join(__dirname, "../build", iconName);
  try {
    ...
    const contextMenu = import_electron13.Menu.buildFromTemplate([
      ...
      },
      {
        ...
        click: () => {
          import_electron13.shell.openPath(import_path10.default.join(import_electron13.app.getPath("userData"), "logs"));
        }
      },
      { type: "separator" },
      {
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    import_electron15.app.setAsDefaultProtocolClient("aipromptlibrary", process.execPath, [import_path11.default.resolve(process.argv[1])]);
  }
} else {
  import_electron15.app.setAsDefaultProtocolClient("aipromptlibrary");
}
...
if (!gotTheLock) {
  ...
  import_electron15.app.on("second-instance", (event, commandLine, workingDirectory) => {
    ...
    if (mainWindow) {
      ...
      mainWindow.focus();
      const url2 = commandLine.find((arg) => arg.startsWith("aipromptlibrary://"));
      if (url2) {
        logger.info(`Received deep link: ${url2}`);
        mainWindow.webContents.send("app:deep-link", url2);
      }
...
import_electron15.app.on("open-url", (event, url2) => {
  event.preventDefault();
  logger.info(`Received deep link (macOS): ${url2}`);
  if (mainWindow) {
    if (mainWindow.isMinimized()) mainWindow.restore();
    mainWindow.focus();
    mainWindow.webContents.send("app:deep-link", url2);
...
import_electron15.app.whenReady().then(async () => {
  ...
  import_electron15.protocol.handle("app", (request) => {
    ...
    let decodedPath = decodeURIComponent(url2.pathname);
    if (decodedPath === "/" || decodedPath === "") {
      decodedPath = "/index.html";
    }
    const outDir = import_path11.default.normalize(import_path11.default.join(__dirname, "../out"));
    let targetPath = import_path11.default.normalize(import_path11.default.join(outDir, decodedPath));
    ...
    if (!import_fs4.default.existsSync(targetPath)) {
      ...
      } else if (import_fs4.default.existsSync(import_path11.default.join(targetPath, "index.html"))) {
        targetPath = import_path11.default.join(targetPath, "index.html");
      } else {
        targetPath = import_path11.default.join(outDir, "index.html");
      }
    }
    return import_electron15.net.fetch((0, import_url2.pathToFileURL)(targetPath).toString());
  });
...
SQLite Path: ${import_path11.default.join(import_electron15.app.getPath("userData"), "prompt_library.db")}
------------------------------
`);
  setupCrashHandler();
  initSettings();
...
import_electron15.ipcMain.handle("app:openExternal", (_event, url2) => {
  const safeUrl = new URL(url2);
  if (safeUrl.protocol === "https:" || safeUrl.protocol === "http:") {
    import_electron15.shell.openExternal(url2);
  }
});
